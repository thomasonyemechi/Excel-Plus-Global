...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
					<div class="breadcrumb-title pe-3">Incentive</div>
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								<li class="breadcrumb-item"><a href="javascript:;"><i class="bx bx-home-alt"></i></a>
								</li>
								<li class="breadcrumb-item active" aria-current="page">Incentive Ledger</li>
							</ol>
						</nav>
					</div>
				</div>					
...
<div class="card">
					<div class="card-body">
						<div class="card-title">
							<h5 class="mb-0">Incentive Ledger</h5>
						...
						<hr/>
                               ...
                               <div class="table-responsive mt-3">
								  <table id="example" class="table table-bordered table-striped table-sm">
                                   ...
                                   <?php
                                          $i=1;
                                          $sql = $db->query("SELECT * FROM wallet WHERE id='$uid' AND type=14 ORDER BY sn DESC LIMIT 200 "); 
                                          while($row = $sql->fetch_assoc()) { $e = $i++;
                                          $opt = explode('|',$row['opt']);
                                          $item = $opt[0] ?? ''; $amt = $opt[1] ?? 0;
                                      ?>
                                        <tr>
                                          <td><?php echo $e; ?></td>
