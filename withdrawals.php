...
<div class="card">
					<div class="card-body">
						<div class="card-title"><div class="ms-auto" style="float: right;">
										<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Withdraw Funds</button>
									</div>
							<h5 class="mb-0">Withdrawal Ledger</h5>
						...
						<hr/>
                               ...
                               <div class="table-responsive mt-3">
								  <table id="example" class="table table-bordered table-striped table-sm">
                                   ...
                                   <?php
                                          $i=1;
                                          $sql = $db->query("SELECT * FROM wallet WHERE id='$uid' AND (type=1 OR type=4) ORDER BY sn DESC LIMIT 200 "); 
	...
	<script>
		...
		var v = document.getElementById('amount').value;
			if(v>4){
 $('#veri').html('<button type="submit" name="FundWithdrawal" class="btn btn-danger">Proceed to Withdraw $'+v+'</button>');
 $('#er').html(''); }
 else{$('#er').html('Minimum withdrawal amount is $5'); }

		}

		function pro(){
			 $('#veri').html('<button type="button" class="btn btn-primary" onclick="proc()">Withdraw Funds </button>');
		}
	</script>
